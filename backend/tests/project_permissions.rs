...
fn actor(role: Option<task_scheduler_backend::domain::project_permissions::ProjectRole>) -> ProjectActor {
    ProjectActor { is_owner: false, role }
}
...
fn removal_matrix_protects_owner_and_peer_managers() {
    let owner = ProjectActor { is_owner: true, role: Some(Manager) };
...
fn graphql_contract_exposes_explicit_transfer_and_timesheet_target() {
    ...
    assert!(schema.contains("transfer_project_ownership(project_id: ID!, new_owner_user_id: ID!): Boolean!"));
    assert!(schema.contains("my_timesheet_entries(project_id: ID!, from: NaiveDate!, to: NaiveDate!, user_id: ID)"));
