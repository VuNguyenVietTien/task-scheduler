...
pub async fn update_multiple_members(
    ...
    if !is_admin {
        eprintln!("User {} lacks manager/leader role in project {}", current_user_id, project_uuid);
        return Err(AppError::forbidden("You must be a manager or leader to update member roles").to_graphql_error());
    }
