...
use crate::graphql::resolvers::members::types::{
    BulkUpdateResponse, MemberRole, MemberRoleUpdate, ProjectMember, UserResponse,
};
...
pub async fn update_multiple_members(
    ...
    // Check if user has manager or leader role (can manage members)
    ...
    if !is_admin {
        eprintln!(
            "User {} lacks manager/leader role in project {}",
            current_user_id, project_uuid
        );
        return Err(
            AppError::forbidden("You must be a manager or leader to update member roles")
                .to_graphql_error(),
        );
    }
    ...
    // Debug log
    eprintln!(
        "Starting bulk update for {} members in project {:?}",
        updates.len(),
        project_id
    );
    ...
    for update in updates {
        ...
        eprintln!(
            "Processing update: user_id={:?}, role={:?}",
            update.user_id, update.role
        );
        ...
        if let Some(member) = result {
            ...
            eprintln!(
                "No member found with user_id={} in project={}",
    ...
    eprintln!(
        "Bulk update completed: updated {} out of {} members",
        success_count, updates_count
    );
