...
pub mod project_authz {
    ...
    async fn actor_tx(
        ...
        Ok(ProjectActor { is_owner, role: role.as_deref().and_then(ProjectRole::parse) })
    }
    ...
    pub async fn require_project_manager_tx(
        ...
        if !actor_tx(tx, user_id, project_id).await?.can_view_settings() {
            return Err(coded_error("Only a project manager may manage settings", "FORBIDDEN"));
        }
        Ok(())
    }

    pub async fn require_member_list(
        pool: &PgPool,
        user_id: Uuid,
        project_id: Uuid,
    ) -> Result<()> {
        ...
        .map_err(AuthError::Database)?;
        let (is_owner, role) = row.ok_or_else(|| coded_error("project not found", "NOT_FOUND"))?;
        let actor = ProjectActor { is_owner, role: role.as_deref().and_then(ProjectRole::parse) };
        if !actor.can_view_members() {
            return Err(coded_error("Guests cannot view project members", "FORBIDDEN"));
        }
        Ok(())
    }
    ...
    pub async fn require_role_assignment_tx(
        ...
        if !actor_tx(tx, caller, project_id).await?.can_assign_role(role) {
            return Err(coded_error("You cannot assign this project role", "FORBIDDEN"));
        }
        Ok(())
    }
    ...
    pub async fn require_member_removal_tx(
        ...
        if !actor.can_remove(target_is_owner, target_role, target == Some(caller)) {
            return Err(coded_error("You cannot remove this project member", "FORBIDDEN"));
        }
        Ok(())
    }
    ...
    pub async fn require_timesheet_access(
        ...
        .await
        .map_err(AuthError::Database)?;
        let (is_owner, role, target_has_access) = row.ok_or_else(|| coded_error("project not found", "NOT_FOUND"))?;
        let actor = ProjectActor { is_owner, role: role.as_deref().and_then(ProjectRole::parse) };
        let allowed = target_has_access && if edit { actor.can_edit_timesheet(caller == target) } else { actor.can_view_timesheet(caller == target) };
        if !allowed {
            return Err(coded_error("You cannot access this member's timesheet", "FORBIDDEN"));
        }
        Ok(())
    }
