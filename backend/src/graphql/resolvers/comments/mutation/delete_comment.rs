...
pub async fn delete_comment(pool: &PgPool, user_id: Uuid, comment_id: &str) -> Result<bool, Error> {
    info!("Attempting to delete comment: {}", comment_id);

    let comment_uuid = Uuid::parse_str(comment_id).map_err(|_| Error::new("Invalid comment ID"))?;

    // Check if comment exists and user has permission to delete
    let comment = sqlx::query("SELECT * FROM comments WHERE comment_id = $1 AND NOT is_deleted")
        .bind(comment_uuid)
        .fetch_optional(pool)
        .await
        .map_err(|e| {
    ...
    if comment_user_id != user_id {
        info!(
            "Permission denied: User {} tried to delete comment {} owned by {}",
    ...
    let now = Utc::now();
    let result =
        sqlx::query("UPDATE comments SET is_deleted = true, updated_at = $1 WHERE comment_id = $2")
            .bind(now)
            .bind(comment_uuid)
            .execute(pool)
            .await
