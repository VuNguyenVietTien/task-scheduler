...
impl ResourceMemberMutation {
    ...
    async fn set_project_member_access(
        ...
        if let Some(ref value) = role {
            ...
            project_authz::require_role_assignment_tx(&mut tx, caller_id, project_id, value).await?;
        }
        sqlx::query(
    ...
    async fn transfer_project_ownership(
        ...
        if caller == new_owner {
            return Err(coded_error("new owner must be another project member", "BAD_USER_INPUT"));
        }
        let mut tx = context.db.begin().await.map_err(AuthError::Database)?;
        let current_owner: Uuid = sqlx::query_scalar(
            "SELECT owner_id FROM projects WHERE project_id = $1 FOR UPDATE",
        )
        .bind(project_id)
        .fetch_optional(&mut *tx)
        .await
        ...
        if current_owner != caller {
            return Err(coded_error("Only the project owner may transfer ownership", "FORBIDDEN"));
        }
        ...
        .map_err(AuthError::Database)?;
        if !eligible {
            return Err(coded_error("new owner must be a linked project member", "BAD_USER_INPUT"));
        }
        sqlx::query("UPDATE projects SET owner_id = $2, updated_at = now() WHERE project_id = $1")
            ...
            .await
            .map_err(AuthError::Database)?;
        sqlx::query("UPDATE project_members SET role = 'manager', updated_at = now() \
