...
pub async fn get_report_tasks(
    ...
    sqlx::query("SELECT * FROM report_tasks WHERE report_id = $1")
        .bind(report_id)
        .map(|row: PgRow| {
            Ok(ReportTask {
                id: row.try_get("id")?,
...
pub async fn get_bugs_by_project(pool: &PgPool, project_id: Uuid) -> Result<Vec<Bug>, sqlx::Error> {
    sqlx::query("SELECT * FROM bugs WHERE project_id = $1")
        .bind(project_id)
        .map(|row: PgRow| {
            Ok(Bug {
                id: row.try_get("id")?,
...
pub async fn create_report(
    ...
    resolved_bugs: i32,
    rejected_tasks: i32,
    on_schedule_percentage: Option<f64>,
    ...
    sqlx::query(
        "INSERT INTO reports (
            ...
            rejected_tasks, on_schedule_percentage, delay_percentage,
    ...
    .bind(resolved_bugs)
    .bind(rejected_tasks)
    .bind(on_schedule_percentage)
    .bind(delay_percentage)
    .bind(summary)
    .bind(now)
    .bind(now)
    .map(|row: PgRow| {
...
pub async fn update_report(
    ...
    .bind(rejected_tasks.unwrap_or(report.rejected_tasks))
    .bind(on_schedule_percentage.or(report.on_schedule_percentage))
    .bind(delay_percentage.or(report.delay_percentage))
