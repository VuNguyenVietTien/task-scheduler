...
impl ProjectActor {
    pub fn can_view_members(self) -> bool {
        self.is_owner || matches!(self.role, Some(ProjectRole::Manager | ProjectRole::Leader | ProjectRole::Member))
    }
    ...
    pub fn can_remove(self, target_is_owner: bool, target_role: Option<ProjectRole>, is_self: bool) -> bool {
        if target_is_owner {
            return false;
        }
        if self.is_owner {
        ...
        match self.role {
            ...
            Some(ProjectRole::Leader) => is_self || matches!(target_role, None | Some(ProjectRole::Member | ProjectRole::Guest)),
            _ => false,
        }
    }

    pub fn can_view_timesheet(self, own: bool) -> bool {
        own || self.is_owner || matches!(self.role, Some(ProjectRole::Manager | ProjectRole::Leader))
    }
