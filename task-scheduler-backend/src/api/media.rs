...
async fn upload_file(mut payload: Multipart) -> Result<HttpResponse, Error> {
    ...
    while let Ok(Some(mut field)) = payload.try_next().await {
        ...
        if field_name == "storage_type" {
            ...
            let content_type = field.content_type().map(|ct| ct.to_string()).unwrap_or_else(|| {
            ...
            if data.len() > 5 * 1024 * 1024 { // 5MB
                ...
                println!("LỖI: {}", error_msg);
                println!("Kích thước file: {} bytes, vượt quá giới hạn 5MB", data.len());
                return Err(error::ErrorBadRequest(error_msg));
            }
    ...
    println!("{}", serde_json::to_string_pretty(&fields_info).unwrap_or_default());
    ...
    if let Some((id, filename, mimetype, size, url)) = file_data {
        ...
        println!("{}", serde_json::to_string_pretty(&error_response).unwrap_or_default());
