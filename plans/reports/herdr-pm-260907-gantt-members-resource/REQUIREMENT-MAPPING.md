...
| Hard-delete task subtree and reject-as-delete | confirmed Delete in detail/List; REJECTED routes to irreversible delete in detail/List/Kanban; Redux removes returned subtree IDs | authorized transaction deletes the locked same-project subtree and returns `project_id` plus `deleted_task_ids`; REJECTED cannot persist | combined clone/delete frontend suites: 35/35; Ubuntu contract: 33/33 | LIVE |
