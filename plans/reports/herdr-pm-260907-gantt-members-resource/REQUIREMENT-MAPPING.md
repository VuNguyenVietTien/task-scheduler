...
| Rename project | Settings name form, refresh project/sidebar | `update_project` returns authoritative name | 12 combined member/rename frontend cases; backend contract | LIVE backend; frontend deployment `db31fcb` being verified |
| Remove linked/unlinked member | per-row Members action, confirm and refresh | `remove_resource_member(project_id, member_id)` transaction and owner/target guard | same 12 frontend cases; Ubuntu contract build | LIVE backend; frontend deployment `db31fcb` being verified |
| Manager catalog/access writes | existing catalog and access editors | catalog mutations / `set_project_member_access`; authorization diagnosis | focused tests pending | IN PROGRESS |
| Delete Progress/Category/Task type | per-item confirmed Delete and task refresh | transaction nulls matching task catalog ID + legacy scalar, deletes item | three-kind unit/contract pending | IN PROGRESS |
